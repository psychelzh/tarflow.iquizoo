...
for (row in seq_len(nrow(games))) {
  if (!hasName(config, games$game_name_abbr[[row]])) {
    config[[games$game_name_abbr[[row]]]] <- list(
      game_name = games$game_name[[row]],
      prep_fun_name = games$prep_fun_name[[row]],
      input = NULL,
      extra = NULL
    )
  }
