...
tar_option_set(
  package = c("tidyverse", "preproc.iquizoo"),
  format = "qs",
...
targets_data <- tarchetypes::tar_map(
  ...
  tar_target(
    scores,
    tarflow.iquizoo::pickup_single_game(
  ...
  tar_target(data_parsed, tarflow.iquizoo::wrangle_data(data, .key = key)),
  tar_target(
    indices,
    tarflow.iquizoo::preproc_data(data_parsed, .fn = prep_fun)
  )
)
```
